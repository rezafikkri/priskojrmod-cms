...
export const contentCustomSchema = z.intersection(
  ...
  z.custom((val) => {
    if (/<p>(&nbsp;|\s)*<\/p>/.test(val)) return false;
    return true;
  }, {
...
export const firstNameSchema = z.string().trim()
  .min(2, { message: 'Must contain at least 2 characters' })
  .max(100, { message: 'Must contain at most 100 characters' });

export const lastNameSchema = z.string().trim()
  .min(2, { message: 'Must contain at least 2 characters' })
  .max(100, { message: 'Must contain at most 100 characters' })
...
export const currencyCodeSchema = z.enum(Object.values(CurrencyCode));
