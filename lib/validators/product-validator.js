...
import { CurrencyCode, PriceType, ProductStatus, Language } from '@/constants/enums';
...
export function withDownloadUrlRefine({ schema, applicationCategoryId }) {
  return schema.refine(
    data => {
      if (
        (data.categoryId === applicationCategoryId || data.priceType === PriceType.FREE) &&
        data.downloadUrl === ''
      ) return false;
...
export function withVersionRefine({ schema, applicationCategoryId }) {
  return schema.refine(
    data => {
      if (data.categoryId === applicationCategoryId) {
        return /^((0|[1-9][0-9]*)\.){2}(0|[1-9][0-9]*)$/.test(data.version);
      }
...
// for edit mode only
export function withChangelogSuperRefine({ schema, version, dbVersion }) {
...
export const createProductExtrasSchema = z.object({
  variants: createProductVariantSchema
    ...
    .array()
    .min(1)
    .superRefine(variantsSuperRefine),
...
export const editProductExtrasSchema = z.object({
  variants: createProductVariantSchema
    ...
    .refine(
      data => data.dbId || data.id,
      { message: 'Variant must have either a dbId or an id' },
    )
    .array()
    .min(1)
    .superRefine(variantsSuperRefine),
...
const pricesRequiredWhenPaidSuperRefine = (data, ctx) => {
  if (data.priceType === PriceType.PAID) {
...
export const createProductSchema = createProductBasicSchema
  ...
  .superRefine(pricesRequiredWhenPaidSuperRefine);
