...
/* Extract social media idetifier from social media profile link
 * 
 ...
 */
export function extractSMIdentifier(SMProfileLink) {
  try {
    ...
    const url = new URL(SMProfileLink);
    const username = url.pathname.replace(/^\/+|\/+$/g, '');
    const useAtSign = atSignHostnames.includes(url.pathname);

    if (useAtSign) return `@${username}`;
    return username;
  } catch (err) {
    return null;
  }
