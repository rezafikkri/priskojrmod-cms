...
export function getNestedValue(obj, path) {
  return path.split('.').reduce((acc, cv) => acc?.[cv] , obj);
}

// Utility to get error info on the form, related to the multi-language feature (id/en).
export function getLangErrorInfo({ activeLang, errors, fieldName }) {
  const parentFieldName = fieldName.slice(0, fieldName.lastIndexOf('.'));
  let isInactiveLangError = false;
  let isActiveLangError = false;
  let error;
  if (errors && Object.keys(errors).length) {
    ...
    if (error && !error[activeLang]) {
      isInactiveLangError = true;
    } else if (error) {
      isActiveLangError = true;
    }
  }

  const inactiveLangErrorMessage = `There's an error in ${activeLang === Language.ID ? 'English' : 'Indonesian'}`
