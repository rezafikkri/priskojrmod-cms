...
export async function getCustomers({ select, pageIndex, pageSize, filters }) {
  ...
  try {
    ...
    return customers.map(({ first_name, last_name, ...rest }) => ({
...
export async function getCustomersForAutocomplete(key, limit = 10) {
  ...
  try {
    ...
    return customers.map(({ first_name, last_name, email, ...rest }) => ({
      ...rest,
      displayLabel: generateNameIdentifierLabel(first_name, last_name, email),
    }));
  } catch (err) {
    console.error(err);
    throw new UnknownError();
...
export async function deleteCustomer(id) {
  ...
  try {
    ...
    const isEligibleToDelete = ((!customer.google_user_id || isInactive) && !hasLicenseKey) || customer.is_banned;
