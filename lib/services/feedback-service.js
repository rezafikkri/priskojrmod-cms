...
export async function pullFeedbacks() {
  ...
  try {
    ...
    for (const row of rowsIndo) {
      ...
      if (row.get('Email')) data.email = row.get('Email').substring(0, 254) || null;
      if (row.get('Nama')) data.name = row.get('Nama').substring(0, 100) || null;
    ...
    for (const row of rowsEn) {
      ...
      if (row.get('Email')) data.email = row.get('Email').substring(0, 254) || null;
      if (row.get('Name')) data.name = row.get('Name').substring(0, 100) || null;
      createData.push(data);
    }
