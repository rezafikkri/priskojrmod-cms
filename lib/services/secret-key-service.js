...
export async function createSecretKey({
  ...
  try {
    ...
    if (!product) {
      throw new NotFoundError('Secret key cannot be created because the selected application is no longer available. Please reload the page and try again.');
    }
...
export async function saveRegeneratedSecretKey({
  ...
  try {
    ...
    const result = await prisma.secretKey.update({
      ...
      data: {
        key: parsedData.key,
        regeneratedAt: getUnixTimestamp(),
      },
      select: { key: true },
    });
