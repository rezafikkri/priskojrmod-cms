...
export async function updateLicense({
  ...
  try {
    ...
    if (err.name === 'PrismaClientKnownRequestError' && err.code === 'P2025') {
      throw new NotFoundError('Failed to update the license because it was not found.');
    }
...
export async function deleteLicense(id) {
  ...
  try {
    ...
    if (err.name === 'PrismaClientKnownRequestError') {
      if (err.code === 'P2025') {
        throw new NotFoundError('Failed to delete the license because it was not found. Please reload the page and try again.');
      }
