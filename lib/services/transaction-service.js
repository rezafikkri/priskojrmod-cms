...
import { getSubtotal, getUnixTimestamp } from '../utils';
...
function buildFilterWhereClause({ searchKey, ...filters }) {
  let where;
  ...
  if (searchKey) {
    const parsedSearchKey = searchKeySchema.parse(searchKey);
    where = { code: parsedSearchKey, ...where };
  }
