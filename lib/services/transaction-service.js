...
// only for change/update status, or correct status with null share_method
  ...
  for (const detail of transaction.details) {
    if (isApplicationCategory(detail.productCategorySlug)) {
      ...
      if (detailLicenseKey.length <= 0) {
        ...
        // TODO: pertimbangkan untuk menghapus logic ini, karena di sisi logic
        // checkout atau pembelian, kita akan menambahkan mekanisme, supaya customer
        // yang sudah punya license-key, tidak bisa checkout, tetapi dia harus/cukup
        // lakukan upgrade license-key expired at saja, melalui halaman yang sudah di sediakan.
