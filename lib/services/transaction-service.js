...
import { Prisma as PrismaPJME } from '@/prisma-pjme-db/pjme-db-client';
...
export async function generateTransactionExport({ transactionStatus, currencyCode }) {
  ...
  try {
    const transactions = await pjmeDBPrismaClient.$queryRaw(PrismaPJME.sql`
      ...
      WHERE td.product_currency_code = ${currencyCode}::currency_code ${
        transactionStatus !== 'all'
          ? PrismaPJME.sql`AND t.status = ${transactionStatus}::transaction_status`
          : PrismaPJME.sql`AND t.status != ${TransactionStatus.PENDING}::transaction_status`
      }
    ...
    return new ReadableStream({
      async start(controller) {
        ...
        const notes = [
          '# Notes:',
          '# - Discount and coupon values are represented as decimals (example: 0.10 = 10%)',
          '"# - Subtotal represents the total price per product item after applying quantity, discount, and coupon."',
          '#   Formula: (unit_price * quantity * (1 - discount) * (1 - coupon))',
          '# - Total Amount represents the total value of the entire transaction (sum of all subtotals in the same transaction)',
