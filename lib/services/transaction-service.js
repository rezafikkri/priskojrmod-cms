...
function assertCustomerActive(customer, targetStatus) {
  if (!customer) {
    throw new NotAllowedError(`The customer account associated with this transaction has been deleted, so the transaction status cannot be changed to ${targetStatus}`);
  }
...
function assertNormalTransactionAllowed(transaction, targetStatus) {
  assertCustomerActive(transaction.customer, targetStatus);
...
async function changeTransactionToPaid({
  ...
  // check product deleted and secret key missing
  ...
  if (deletedProducts.length > 0) {
    throw new NotAllowedError(`This transaction includes deleted products: ${formatList(deletedProducts)}. Please ask customer to create a new transaction.`);
  }
