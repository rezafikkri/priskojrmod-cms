...
import { getUnixTimestamp, toTitleCase } from '../utils';
import { getSubtotalBreakdown } from '../pricing';
...
import {
  ...
  verifyOrDecodeExpiredLicenseKey,
...
function buildFilterWhereClause(filters) {
  ...
  if (parsedFilters?.adminId) where = { adminId: parsedFilters.adminId, ...where };
      ...
      select: {
        ...
        details: {
          select: {
            ...
            productUpgradeCouponCode: true,
            productUpgradeCouponDiscount: true,
    ...
    if (transaction) {
      ...
      return {
        ...
        details: details.map(detail => {
          const { subtotal, discountAmount, upgradeCouponAmount } = getSubtotalBreakdown({
            price: detail.productPrice,
            qty: detail.qty,
            currencyCode: detail.productCurrencyCode,
            discount: detail.productDiscount,
            upgradeCouponDiscount: detail.productUpgradeCouponDiscount,
          });
          const result = {
            ...detail,
            productPrice: detail.productPrice.toString(),
            subtotal: subtotal.toString(),
          };

          if (discountAmount) result.discountAmount = discountAmount.toString();
          if (upgradeCouponAmount) result.upgradeCouponAmount = upgradeCouponAmount.toString();
...
/**
 * Builds license key create data. Used only in `changeTransactionToPaid`
 * (called only on first transition to "paid").
 */
function buildLicenseKeyCreateData({
  transaction,
  secretKey,
  currentTime,
}) {
...
/**
 * Builds license key update data. Used only in `changeTransactionToPaid`
 * (called only on first transition to "paid").
 */

function buildLicenseKeyUpdateData({
  transaction,
  secretKey,
  licenseKey,
  currentTime,
}) {
  const parsedOldCode = verifyOrDecodeExpiredLicenseKey(
    licenseKey.code,
    secretKey.key,
  );
  const baseTime = (!licenseKey.isRevoked && parsedOldCode.exp > currentTime)
    ? parsedOldCode.exp
    : currentTime;
  ...
  const newCode = generateLicenseKeyCode({
    payload: {
  ...
  const payload = {
    ...
    code: newCode,
    updatedAt: currentTime,
    oldUpdatedAt: licenseKey.updatedAt,
    oldCode: licenseKey.code,
  };

  if (licenseKey.isRevoked) {
    payload.isRevoked = false;
  }
...
/**
 * IMPORTANT: Only call this on the transaction's FIRST transition to "paid".
 * Valid triggers: pending -> paid, or pending -> cancelled -> paid (status correction).
 *
 * This function change the app license-key's expired_at to.
 * Calling it on any other status transition will incorrectly overwrite expired_at.
 */
async function changeTransactionToPaid({
  ...
  for (const detail of transaction.details) {
    if (isApplicationCategory(detail.productCategorySlug)) {
      const secretKey = secretKeyMap.get(detail.productId);
      const licenseKeys = secretKey.licenseKeys;

      // if license key doesn't exist
      if (licenseKeys.length <= 0) {
        newLicenseKeys.push(buildLicenseKeyCreateData({
          transaction,
          secretKey,
          currentTime,
        }));
      } else {
        licenseKeysToUpdate.push(buildLicenseKeyUpdateData({
          transaction,
          secretKey,
  ...
  if (licenseKeysToUpdate.length > 0) {
    await updateLicenseKeys({
      data: licenseKeysToUpdate.map(({ oldCode, oldUpdatedAt, ...rest }) => rest),
      rollback: async () => {
        if (newLicenseKeys.length > 0) {
  ...
  // create invoice and update transaction status and transaction details
  const processResult = await processTransactionPaidUpdate({
    ...
    rollback: async () => {
      if (licenseKeysToUpdate.length > 0) {
        await updateLicenseKeys({
          data: licenseKeysToUpdate.map(({ oldCode, code, oldUpdatedAt, ...rest }) => {
            const licenseKey = {
              ...rest,
              code: oldCode,
  ...
  const result = {
    invoice: { invoiceNumber: processResult.invoiceNumber },
    paidAt: currentTime,
    updatedAt: currentTime,
...
async function changeTransactionToRefund({
  ...
  for (const detail of transaction.details) {
    if (detail.shareMethod === ShareMethod.DRIVE_SHARE) {
      ...
      // revoke license key
      if (licenseKeys && licenseKeys.length > 0 && !licenseKeys[0].isRevoked) {
        licenseKeysToUpdate.push({
          id: licenseKeys[0].id,
          isRevoked: true,
          revokeNote: '[System] Revoked because the related transaction status changed to refund.',
          updatedAt: currentTime,
        });
      }
  ...
  await prisma.$transaction(async (tx) => {
    ...
    if (existingInvoice) {
      await tx.invoice.update({
        where: { id: existingInvoice.id },
        data: { status: InvoiceStatus.REFUND },
        select: { id: true },
      });
    }
...
// ensure flowType is hard code, not from client
async function prepareTransactionContext({
  ...
  if (!transaction) {
    const statusAction = flowType === 'correct' ? 'corrected' : 'changed';
    throw new NotFoundError(`Transaction status cannot be ${statusAction} because the selected transaction is no longer available. Please refresh the table and try again.`);
  }

  // validate status that want to change
  if (flowType === 'correct') {
    assertCorrectTransactionAllowed(transaction, targetStatus);
  } else {
    assertNormalTransactionAllowed(transaction, targetStatus);
  }
...
async function correctTransactionToPaid({
  ...
  for (const detail of transaction.details) {
    if (detail.shareMethod === ShareMethod.DRIVE_SHARE) {
      ...
      if (licenseKeys && licenseKeys.length > 0 && licenseKeys[0].isRevoked) {
        licenseKeysToUnrevoke.push(licenseKeys[0].id);
      }
  ...
  // create invoice, update transaction status, transaction details and clear refund fields
    ...
    if (detail.shareMethod === ShareMethod.DRIVE_SHARE) {
      ...
      if (licenseKeys && licenseKeys.length > 0 && !licenseKeys[0].isRevoked) {
        licenseKeysToUpdate.push({
          id: licenseKeys[0].id,
          isRevoked: true,
          revokeNote: '[System] Revoked because the related transaction status was corrected to cancelled.',
          updatedAt: currentTime,
        });
      }
  ...
  try {
    ...
    const query = Prisma.sql`
      SELECT
        ...
        td.product_discount,
        td.product_upgrade_coupon_code,
        td.product_upgrade_coupon_discount
    ...
    return new ReadableStream({
      async start(controller) {
        ...
        const notes = [
          ...
          '"# - Admin fields refer to the admin assigned to this transaction"',
        ].join('\n') + '\n\n';
        ...
        const stringifier = stringify({
          ...
          columns: [
            ...
            { key: 'productUpgradeCouponCode', header: 'Upgrade Coupon Code' },
            { key: 'productUpgradeCouponDiscount', header: 'Upgrade Coupon %' },
        ...
        for (const row of transactions) {
          const { subtotal, discountAmount, upgradeCouponAmount } = getSubtotalBreakdown({
            ...
            upgradeCouponDiscount: row.product_upgrade_coupon_discount,
          });
          ...
          stringifier.write({
            ...
            productUpgradeCouponCode: row.product_upgrade_coupon_code ?? '',
            productUpgradeCouponDiscount: row.product_upgrade_coupon_discount ?? '',
            upgradeCouponAmount: upgradeCouponAmount ? `-${upgradeCouponAmount.toString()}` : '',
