...
import { ShareMethod, TransactionStatus } from '@/constants/enums';
import NotAllowedError from '../errors/NotAllowedError';
import pjmaDBPrismaClient from '../pjma-prisma-client';
import { v7 as uuidv7 } from 'uuid';
import { createLicenseKeys, deleteLicenseKeys, generateLicenseKeyCode, updateLicenseKeysRevokeStatus } from './license-key-service';
import { getDriveFileInfo } from './product-service';
import { InvoiceStatus } from '@/prisma-pjme-db/pjme-db-client';
...
async function checkDriveFile(detailProduct, sharedFiles) {
  try {
    await getDriveFileInfo(detailProduct.product_drive_file_id);
    return true;
  } catch (err) {
    if (err.name === 'UnavailableError' || err.name === 'NotFoundError') {
      ...
      throw NotAllowedError(`Product ${detailProduct.name} has no accessible file. Google Drive file is missing or in trash.`);
    }
...
async function createInvoice({
  ...
  try {
    ...
    if (!(err instanceof EcommercePrisma.PrismaClientKnownRequestError && err.code == 'P2002')) {
      throw err;
    }
  }
}
...
export async function updateTransactionStatus({
  ...
  try {
    ...
    if (!checkUpdateStatus(transaction.status, parsedStatus)) {
      ...
      throw new NotAllowedError('The customer owning this transaction is banned, so the transaction status cannot be changed to paid.');
    }
    ...
    if (parsedStatus === TransactionStatus.PAID) {
      ...
      for (const detail of transaction.details) {
        ...
        } else if (detailProduct.product_drive_file_id) {
          const isDriveFileAccessible = await checkDriveFile(detailProduct, sharedFiles);
      ...
      return await processTransactionPaidUpdate({
