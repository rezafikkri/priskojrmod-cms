...
async function changeTransactionToRefund({
  ...
  // if currentTime > refundDeadline, then throw
  const refundDeadline = transaction.paidAt + (60 * 60 * 24 * 7);
  if (currentTime > refundDeadline) {
    throw new NotAllowedError('Refund is no longer available for this transaction. The allowed refund period of 7 days after purchase has passed.');
  }
