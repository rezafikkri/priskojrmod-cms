...
function buildProductOperationWithPriceFlag({ parsedData, isVersionChanged }) {
  const currentTime = getUnixTimestamp();
  const slug = parsedData.name.toLowerCase().replace(/\s/g, '-');
...
function buildDiscountOperation({ discount, isPaid, productId }) {
  if (!isPaid) return [];
...
function buildUpgradeCouponOperation({ upgradeCoupon, isPaid, productId, isVersionChanged}) {
  if (!isPaid) return [];
...
export async function updateProduct({
  ...
  try {
    ...
    const { operation, isPaid } = buildProductOperationWithPriceFlag({
      parsedData,
      isVersionChanged,
    });
    let transactionItems = [
      operation,
      ...buildDiscountOperation({ discount: parsedData.discount, isPaid, productId: parsedData.id }),
      ...buildUpgradeCouponOperation({
