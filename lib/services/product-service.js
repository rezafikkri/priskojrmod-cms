...
import { mapTranslationsToObject, getUnixTimestamp, toTitleCase } from '../utils';
    ...
    let createData = {
      ...
      variants: {
        create: parsedData.variants.map(variant => {
          let variantCreateData = {
            name: variant.name,
            translations: {
              create: [
                { language: Language.ID, description: variant.description.id },
                { language: Language.EN, description: variant.description.en },
              ],
            },
          };

          if (variant.downloadUrl) {
            variantCreateData.downloadUrl = variant.downloadUrl;
            variantCreateData.fileAccessPassword = variant.fileAccessPassword;
          }

          if (parsedData.priceType === PriceType.PAID && variant.prices) {
            variantCreateData.prices = {
...
export async function getProducts(filters) {
  ...
  try {
    ...
    return [...pinnedProducts, ...unpinnedProducts].map(({ admin, variants, versions, ...product}) => {
      if (admin) {
      ...
      if (product.priceType === PriceType.PAID) {
        ...
        let priceMinMax = prices.reduce((acc, { currencyCode, price }) => {
          if (!acc[currencyCode]) {
            acc[currencyCode] = { min: price, max: price };
          } else {
            if (acc[currencyCode].min.gt(price)) acc[currencyCode].min = price;
            if (acc[currencyCode].max.lt(price)) acc[currencyCode].max = price;
          }
        ...
        product.prices = Object.fromEntries(
          Object.entries(priceMinMax).map(([currency, range]) => [
            currency,
            {
      ...
      product.releasedAt = versions[0].releasedAt; 
...
function mapProductToFormData(product, mode = 'normal') {
  ...
  for (const variant of product.variants) {
    variants.push({
      ...
      name: variant.name,
      translationId: {
        id: variant.translations.id.id,
        en: variant.translations.id.en,
      },
      description: {
        id: variant.translations.description.id,
        en: variant.translations.description.en,
      },
  ...
  // TODO: logic ini mungkin dihapus, karena sudah ada kanban board yang menugaskan itu,
  // agar variant jika cuma tinggal 1 di halaman edit, maka tidak bisa dihapus lagi.
...
export async function getProduct(id) {
  ...
  try {
    ...
    product.variants = product.variants.map(variant => {
      variant.translations = mapTranslationsToObject(variant.translations);
      return variant;
    });
...
async function deleteProductChild({ 
  ...
  try {
    ...
    if (['productVariant', 'productImage'].includes(entityName)) {
      const childCount = await prisma[entityName].count({
        where: { productId: parsedProductId },
      });

      if (childCount === 1) {
        throw new NotAllowedError(`${toTitleCase(entityLabel)} cannot be deleted because it's the last existing one.`);
      }
    }
        ...
        let upsertVariant = {
          ...
          update: {
            ...
            translations: {
              update: [
                {
                  data: { description: variant.description.id },
                  where: { id: variant.translationId?.id ?? v7() },
                },
                {
                  data: { description: variant.description.en },
                  where: { id: variant.translationId?.en ?? v7() },
                },
              ],
            },
          },
          where: { id: variant.dbId ?? v7() }, 
        };
        
        if (parsedData.priceType === PriceType.PAID && variant.prices) {
          upsertVariant.create.prices = { create: variant.prices };

          upsertVariant.update.prices = {
...
function getDiscountOperation({ discount, isPaid, productId }) {
  if (!isPaid) return [];
...
function getUpgradeCouponOperation({ upgradeCoupon, isPaid, productId, isVersionChanged}) {
  if (!isPaid) return [];
...
export async function updateProduct({
  ...
  try {
    ...
    // paid product cannot changed to free product
    if (product.priceType === PriceType.PAID && parsedData.priceType === PriceType.FREE) {
      throw new NotAllowedError('Paid products cannot be changed to free.');
    }

    const isVersionChanged = product.versions[0].version !== parsedData.version;

    const { operation, isPaid } = getProductOperationWithPriceFlag({
      parsedData,
      isVersionChanged,
    });
    let transactionItems = [
      operation,
      ...getDiscountOperation({ discount: parsedData.discount, isPaid, productId: parsedData.id }),
    ...
    const productFormData = mapProductToFormData(
      {
        ...results[0],
        variants: results[0].variants.map(variant => {
          variant.translations = mapTranslationsToObject(variant.translations);
          return variant;
        }),
        versions: [{
