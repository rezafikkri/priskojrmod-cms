...
export async function getSelectableProducts() {
  const session = await getServerSession(authOptions);
  const select = { id: true, name: true, admin_id: true };

  try {
    // don't select admin_id when is owner
    if (isOwnerAdmin(session.user.role)) delete select.admin_id;
    ...
    return products.map(({ admin_id, ...product}) => ({
      ...product,
      isAssignedToCurrentAdmin: admin_id === session.user.id,
    }));
  } catch (err) {
    console.error(err);
    throw new UnknownError();
...
async function deleteProductChild({ 
  ...
  try {
    ...
    return await prisma.$transaction([
      pjmeDBPrismaClient[entityName].delete({
        where: deleteWhere,
        select: { id: true },
      }),
