      ...
      update: {
        data: {
          translations: {
            upsert: [
              {
                create: { language: Language.ID, changelog: parsedData.changelog.id },
                update: { changelog: parsedData.changelog.id },
                where: { id: parsedData.versionTranslationId?.id ?? v7() },
              },
              {
                create: { language: Language.EN, changelog: parsedData.changelog.en },
                update: { changelog: parsedData.changelog.en },
                where: { id: parsedData.versionTranslationId?.en ?? v7() },             
              },
            ],
          },
        },
...
export async function updateProduct({
  ...
  try {
    ...
    // Main reason for select to db is for validate update category
    const product = await pjmeDBPrismaClient.Product.findUnique({
      where: { id: parsedData.id },
      select: {
        name: true,
    ...
    const shouldUpdateReleasedAt = product.versions[0].version !== parsedData.version;
