...
import { isSemverFormat, mapTranslationsToObject, getUnixTimestamp } from '../utils';
...
function getUpgradeCouponOperation({ upgradeCoupon, hasPriceType, productId, isVersionChanged}) {
  ...
  if (upgradeCoupon?.id) {
    if (upgradeCoupon.code && !isVersionChanged) {
      return [
        prisma.productUpgradeCoupon.update({
          where: { id: upgradeCoupon.id },
          data: {
  ...
  try {
    ...
    const product = await prisma.product.findUnique({
      where: {
        ...
        ...(!isOwner ? { adminId: session.userId } : {}),
      },
      select: {
        name: true,
        versions: {
    ...
    if (isVersionChanged) {
      ...
      if (!changelogIdResult.success || !changelogEnResult.success) {
        throw new NotAllowedError('Changelog is required when the version is changed.');
      }
    }
