...
import { UserRole, CurrencyCode, Language, PriceType, ProductStatus, TransactionStatus } from '@/constants/enums';
...
export async function createProduct({
  ...
  try {
    ...
    const applicationCategory = await prisma.category.findUnique({
      where: { id: parsedData.categoryId, slug: APPLICATION_CATEGORY_SLUG },
      select: { id: true, },
    });
      ...
      if (licenseKeys && licenseKeys.length > 0) {
        ...
        for (const licenseKey of licenseKeys) {
          if (licenseKey.lastUsedAt) {
            ...
            if (Math.floor(new Date().getTime() / 1000) < oneYearAfterLastUsedEpoch) {
              throw new NotAllowedError(`Product cannot be deleted because some license keys for this product were used within the last year${showInactiveOption ? '. You can mark it as inactive for now.' : ''}`);
            }
          }

          if (!isLicenseKeyExpired(licenseKey.code, secretKey.key)) {
            throw new NotAllowedError(`Product cannot be deleted because some license keys for this product have not yet expired${showInactiveOption ? '. You can mark it as inactive for now.' : ''}`);
          }
        }
      }
    }
...
export async function updateProduct({
  ...
  try {
    ...
    const isApplicationCategory = newCategory.slug === APPLICATION_CATEGORY_SLUG;
    ...
    const shouldUpdateCategory = product.category.slug !== APPLICATION_CATEGORY_SLUG &&
      newCategory.slug !== APPLICATION_CATEGORY_SLUG;
