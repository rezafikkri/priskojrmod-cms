...
import { CurrencyCode, Language, PriceType, ProductStatus } from '@/constants/enums';
...
function validateProductRules({ parsedData, isApplicationCategory, adminRole }) {
  ...
  if (!parsedData.adminId && isOwnerAdmin(adminRole)) {
...
export async function createProduct({
  ...
  try {
    ...
    let createData = {
      ...
      adminId: isOwnerAdmin(session.userRole) ? parsedData.adminId : session.userId,
...
export async function getProducts(filters) {
  ...
  try {
    ...
    const pinnedProducts = await prisma.product.findMany({
      where: { isPinned: true, ...prismaWhere },
      select,
      orderBy: { updatedAt: 'desc' },
    });
    const unpinnedProducts = await prisma.product.findMany({
      where: { isPinned: false, ...prismaWhere },
      select,
      orderBy: { updatedAt: 'desc' },
    });
...
export async function getSelectableProducts() {
  ...
  try {
    ...
    if (isOwnerAdmin(session.user.role)) delete select.adminId;
  ...
  try {
    ...
    return await prisma.product.update({
      where: {
        ...
        ...(!isOwnerAdmin(session.userRole) ? { adminId: session.userId } : {}),
      },
      data: {
        status: parsedStatus,
...
export async function updateProductPinned(id, isPinned) {
  ...
  if (!isOwnerAdmin(session.userRole)) throw new NotAllowedError();
