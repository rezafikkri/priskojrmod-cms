...
export async function createCategory({ name }) {
  ...
  try {
    ...
    if (err.name === 'PrismaClientKnownRequestError' && err.code === 'P2002') {
      throw new DuplicateError('Category cannot be created because a category with the same name already exists.');
    }
...
async function assertNotApplicationCategory(id) {
  const appCategory = await prisma.category.findUnique({
    where: { id, slug: APPLICATION_CATEGORY_SLUG },
    select: { id: true },
  });
...
export async function deleteCategory(id) {
  ...
  try {
    ...
    await assertNotApplicationCategory(parsedId);
    ...
    if (err.name === 'PrismaClientKnownRequestError') {
      if (err.code === 'P2025') {
        throw new NotFoundError('Failed to delete the category because it was not found. Please reload the page and try again.');
      }
...
export async function updateCategory({
  ...
  try {
    ...
    await assertNotApplicationCategory(parsedData.id);
    ...
    if (err.name === 'PrismaClientKnownRequestError' && err.code === 'P2025') {
      throw new NotFoundError('Failed to update the category because it was not found.');
    }   
    if (err.name === 'NotAllowedError') {
      throw err;
    }
