...
export async function getCategories(
  ...
  console.log(`from category service. NODE_ENV in server component: ${process.env.NODE_ENV}`);
  try {
    ...
    return categories.map(category => {
      ...
      if (category.created_at) {
        newCategory.created_at = category.created_at.toString();
      }
      if (category.updated_at) {
        newCategory.updated_at = category.updated_at.toString();
      }
      return newCategory;
    });
  } catch (err) {
