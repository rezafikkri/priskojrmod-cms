...
export async function generateInvoicePdf(invoiceNumber) {
  ...
  try {
    ...
    if (invoice.status === InvoiceStatus.CANCELLED) statusColor = '#b91c1c';
    if (invoice.status === InvoiceStatus.UNPAID) statusColor = '#3f3f46';
    if (invoice.status === InvoiceStatus.REFUND) statusColor = '#b45309';
    ...
    invoice.transaction.details.forEach((detail, index) => {
      const { amount, subtotal, discountAmount, upgradeCouponAmount } = getSubtotalBreakdown({
        qty: detail.qty,
      ...
      rows.push({
        ...
        qty: detail.qty,
        unit: formatCurrency({
          value: detail.productPrice.toString(),
          currencyCode: detail.productCurrencyCode,
        }),
      ...
      if (upgradeCouponAmount) {
        rows.push({
          rowGroup: index,
          product: `Upgrade Coupon (${detail.productUpgradeCouponCode}, ${detail.productUpgradeCouponDiscount}%)`,
          isChild: true,
          amount: `-${formatCurrency({
            value: upgradeCouponAmount.toString(),
            currencyCode: detail.productCurrencyCode,
          })}`,
        });
      }
    ...
    pdf.text(
      formatCurrency({
        value: invoice.transaction.totalAmount.toString(),
