...
export async function generateInvoicePdf(invoiceNumber) {
  ...
  try {
    ...
    pdf.setFontSize(12);
    pdf.text(BRAND.name, page.marginX, brandY);

    pdf.setFontSize(11);
    pdf.setFont('Montserrat', 'normal');
    brandY += lineHeight;
    pdf.text(BRAND.url, page.marginX, brandY);

    brandY += lineHeight;
    pdf.text(`(Email) ${aboutUs.supportEmail}`, page.marginX, brandY);
    ...
    invoice.transaction.details.forEach((detail, index) => {
      ...
      rows.push({
        rowGroup: index,
        product: `${detail.productName} - ${detail.productVersion}`,
        qty: detail.quantity,
        unit: formatCurrency({
          value: productPrice,
      ...
      if (detail.productDiscount) {
        let discountPrice = currentAmount * (detail.productDiscount / 100);
        if (detail.productCurrencyCode === CurrencyCode.IDR) discountPrice = Math.round(discountPrice);
      ...
      if (detail.productCouponDiscount) {
        let couponPrice = currentAmount * (detail.productCouponDiscount / 100);
        if (detail.productCurrencyCode === CurrencyCode.IDR) couponPrice = Math.round(couponPrice);
