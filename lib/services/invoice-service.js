...
export async function generateInvoicePdf(invoiceNumber) {
  ...
  try {
    ...
    const issuedDate = dayjs.unix(Number(invoice.issued_at)).utc().format('DD MMMM YYYY');
    ...
    invoice.transaction.details.forEach((detail, index) => {
      ...
      rows.push({
        ...
        qty: detail.quantity,
        unit: formatCurrency(productPrice, detail.product_currency_code),
        amount: formatCurrency(currentAmount, detail.product_currency_code),
      });

      if (detail.product_discount) {
        const discountPrice = currentAmount * (detail.product_discount / 100);
        ...
        rows.push({
          ...
          isChild: true,
          amount: `-${formatCurrency(discountPrice, detail.product_currency_code)}`,
        });
      }

      if (detail.product_coupon_discount) {
        const discountPrice = currentAmount * (detail.product_coupon_discount / 100);
    ...
    pdf.text(
      formatCurrency(invoice.transaction.total_amount.toNumber(), invoice.transaction.currency_code),
      summaryValueX,
      summaryY,
      { align: 'right' },
    );
