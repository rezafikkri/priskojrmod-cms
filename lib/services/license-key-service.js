...
export async function createLicenseKey({
  ...
  try {
    ...
    if (!secretKey) {
      throw new NotFoundError('License key cannot be created because the selected secret key is no longer available. Please reload the page and try again.');
    }
    ...
    } else if (err instanceof NotAllowedError || err instanceof NotFoundError) {
      throw err;
    }
...
export async function getLicenseKey(id) {
  ...
  try {
    let licenseKey = await prisma.licenseKey.findUnique({
      where: { id: parsedId },
      select: {
        id: true,
        code: true,
...
export function verifyOrDecodeExpiredLicenseKey(code, secretKey) {
  try {
...
function buildRegeneratedLicenseKeyCodeIfNeeded({
  parsedOldCode,
  parsedData,
  oldSecretKey,
}) {
...
export async function updateLicenseKey({
  ...
  try {
    ...
    if (!licenseKey) {
      throw new NotFoundError('License key cannot be updated because it no longer exists.')
    }

    // check if license key code need to change or not
    const parsedOldCode = verifyOrDecodeExpiredLicenseKey(licenseKey.code, licenseKey.secretKey.key);
    const newCode = buildRegeneratedLicenseKeyCodeIfNeeded({
      parsedOldCode,
      parsedData,
...
export async function getLicenseKeyRevokeNote(id) {
  await verifySession();
  ...
  try {
    ...
    let revokeNote = await prisma.licenseKey.findUnique({
      where: { id: parsedId },
      select: {
        customer: {
          select: {
    ...
    if (revokeNote) {
      const { customer: { firstName, lastName, email }, ...rn } = revokeNote;

      let revokeNoteMapped = rn;
      revokeNoteMapped.customerName = `${firstName} ${lastName}`;
      revokeNoteMapped.customerEmail = email;
      
      return revokeNoteMapped;
    }
...
export async function updateLicenseKeyRevokeStatus({ id, isRevoked, revokeNote}) {
  await verifySession();
  ...
  try {
    ...
    if (parsedData.isRevoked) updateData.revokeNote = parsedData.revokeNote;
...
// Security Note: `data` must only come from internal system logic,
...
// Security Note: `data` must only come from trusted internal logic,
...
export async function updateLicenseKeys({
  ...
  try {
    return await prisma.$transaction(
      data.map(({ id, ...rest }) => prisma.licenseKey.update({
        where: { id },
        data: rest,
        select: { id: true },
      })),
