...
export default function DetailsSection({ details }) {
  return details.map((detail, index) => {
    let currentAmount = detail.quantity * detail.productPrice;

    let discountPrice;
    let couponPrice;

    if (detail.productDiscount) {
      discountPrice = currentAmount * (detail.productDiscount / 100);
    ...
    if (detail.productCouponDiscount) {
      couponPrice = currentAmount * (detail.productCouponDiscount / 100);
      if (detail.productCurrencyCode === CurrencyCode.IDR) couponPrice = Math.round(couponPrice);

      couponPrice = `- ${formatCurrency({
        value: couponPrice,
        currencyCode: detail.productCurrencyCode,
      })}`;
    }
